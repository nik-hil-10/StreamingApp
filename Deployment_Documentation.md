...
We created a declarative `Jenkinsfile` pipeline that automatically pulls our code from GitHub, logs into AWS ECR, and builds and pushes all 5 microservice images. 

Here is the script we used for our `Jenkinsfile`:
```groovy
pipeline {
    agent any
    environment {
        AWS_DEFAULT_REGION = 'ap-south-1'
        ECR_REGISTRY = '791358130074.dkr.ecr.ap-south-1.amazonaws.com'
    }
    stages {
        stage('ECR Login') {
            steps {
        ...
        stage('Build & Push Streaming Service') {
            steps {
                sh 'docker build -t $ECR_REGISTRY/streaming-streaming:latest ./backend/streamingService'
                sh 'docker push $ECR_REGISTRY/streaming-streaming:latest'
            }
        }
        stage('Build & Push Admin Service') {
            steps {
                sh 'docker build -t $ECR_REGISTRY/streaming-admin:latest ./backend/adminService'
                sh 'docker push $ECR_REGISTRY/streaming-admin:latest'
            }
        }
        stage('Build & Push Chat Service') {
            steps {
                sh 'docker build -t $ECR_REGISTRY/streaming-chat:latest ./backend/chatService'
                sh 'docker push $ECR_REGISTRY/streaming-chat:latest'
            }
        }
        stage('Build & Push Frontend') {
            steps {
                sh 'docker build -t $ECR_REGISTRY/streaming-frontend:latest ./frontend'
